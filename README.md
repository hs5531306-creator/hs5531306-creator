# Hi, I'm Suryansh👋 

<div align="center">

  ![My Skills](https://go-skill-icons.vercel.app/api/icons?i=python,kaggle,numpy,googlecolab,jupyter,excel,jira,github,tableau,pandas,matplotlib,mysql,powerpoint,pbi)


![Header](https://capsule-render.vercel.app/api?type=waving&color=0:B497BD,100:F75C5C&height=200&section=header&text=Hi,%20I'm%20Suryansh%20Singh!%20👋&fontSize=40&fontColor=ffffff&animation=fadeIn&fontAlignY=30&desc=MBA%20(Digital%20Business)%20|%20Business%20%26amp;%20Data%20Analytics&descAlignY=50)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=4000&pause=1000&color=B497BD&center=true&vCenter=true&width=700&lines=Business+Analyst;Data+Analyst;SQL+%7C+Python+%7C+Power+BI;Pre-Sales+%26amp;+Strategy+Consulting;Turning+Data+into+Insights)](https://git.io/typing-svg)

<img src="https://raw.githubusercontent.com/Harb1nger001/harb1nger001/main/assets/hello.gif" width="500"/>

</div>


---

## 🚀 About Me

🎓 MBA (Digital Business) @ IIIT Lucknow  
📊 Data Analytics & Business Intelligence Enthusiast  
💡 Passionate about solving real-world problems using data & AI  
📈 Experience in Pre-Sales Strategy, AI Consulting & Smart City Projects  

---

## 🛠️ Skills & Tools

### 📊 Data & Analytics
- SQL (CTEs, Window Functions, Aggregations)  
- Python (Pandas, NumPy, Scikit-learn)  
- Power BI (Dashboards, KPIs, Visualization)  

### 🧠 Business Analysis
- Requirement Analysis  
- Stakeholder Management  
- BRD, FSD, User Stories  
- Process Mapping (AS-IS / TO-BE)  
- Agile & Scrum  

---

## 💼 Experience

### Tata Consultancy Services (TCS) — *Summer Intern (Pre-Sales & Strategy)*  
📍 Jun 2025 – Aug 2025  

- Contributed to AI City Lucknow strategic consulting proposal  
- Built revenue models & pricing strategies  
- Conducted benchmarking of global AI cities  
- Analyzed AI adoption across sectors  
- Assisted in implementation planning & risk mitigation  
- Created executive-ready pre-sales documentation  

---

## 📂 Projects

### 📊 Sales, Customer & Product Performance Analysis (SQL)
- Monthly & yearly trend analysis using window functions  
- Built KPIs: AOV, recency, segmentation  
- Identified top & underperforming products  

---

### 🏋️ Smart Fitness & Nutrition Analysis (ML)
- Analyzed 20,000+ records  
- Built Ridge, Lasso, ElasticNet models (R² = 0.681)  
- Reduced features from 19 → 6  

---

### 🌍 Global COVID-19 Impact Analysis (SQL)
- Analyzed 5M+ records using CTEs & window functions  
- Derived infection & death KPIs  
- Identified high-risk regions  

---

## 📜 Certifications

- IBM SkillsBuild – Data Analytics  
(Python | SQL | Power BI | Statistical Analysis | BI)

---

## 🌱 Leadership & Activities

- Coordinator – CREATE Incubation Club  
- Head of Administration – MUN  
- Event Coordinator – HERO Changemaker  

---

## 📫 Connect With Me

📧 hs5531306@gmail.com  
💼 LinkedIn: https://linkedin.com/in/YOUR-LINK  

---

<div align="center">

⭐ Turning Data into Business Decisions

</div>
